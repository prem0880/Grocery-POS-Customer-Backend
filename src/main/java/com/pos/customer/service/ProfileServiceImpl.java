...
public class ProfileServiceImpl implements ProfileService {
	...
	public ResponseEntity<String> addState(Long countryCode,State stateDetails) {
		...
		return countryRepository.findById(countryCode)
				...
				}).orElse(new ResponseEntity<String>("Country Not Found With"+" "+countryCode+"!",new HttpHeaders(),HttpStatus.OK));
	}
	@Override
	public ResponseEntity<Customer> getCustomerDetails(Long phoneNo) {
		// TODO Auto-generated method stub
		return new ResponseEntity<Customer>(customerRepository.findById(phoneNo).get(),new HttpHeaders(),HttpStatus.OK);
	}
	@Override
	public ResponseEntity<String> updateCustomer(Long phoneNo,Customer customerDetails) {
		// TODO Auto-generated method stub
		return customerRepository.findById(phoneNo)
				.map(customer->{
					customer.setName(customerDetails.getName());
					customer.setEmail(customerDetails.getEmail());
					customerRepository.save(customer);
					return new ResponseEntity<String>("Customer Details Updated Successfully!",new HttpHeaders(),HttpStatus.OK);
				}).orElse(new ResponseEntity<String>("Customer Not Found With"+" "+phoneNo+"!",new HttpHeaders(),HttpStatus.OK));
	}
	@Override
	public ResponseEntity<String> addAddress(Long customerId, Long stateId, Address addressDetails) {
		...
		{
			response=new ResponseEntity<String>("Customer Not Found With"+" "+customerId+"!",new HttpHeaders(),HttpStatus.OK);
		}
		if(!stateRepository.existsById(stateId))
		{
			response=new ResponseEntity<String>("State Not Found With"+" "+stateId+"!",new HttpHeaders(),HttpStatus.OK);
		}
